...
exports.adminList = async (req, res) => {
  try {
    ...
    res.json({ success: true, events, pagination: { page, total, totalPages: Math.ceil(total / limit) } });
  } catch (error) {
  ...
  try {
    ...
    try {
      if (event.thumbnailUrl && event.thumbnailUrl.includes('.amazonaws.com/')) {
        ...
        if (key) {
          await s3Client.send(new DeleteObjectCommand({ Bucket: process.env.AWS_HUB_EVENT_THUMBNAILS, Key: key }));
        }
      }
    } catch {}
