        ...
        <div className="hub-modal" onClick={(e: MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
          ...
          <div className="hub-modal-content">
            ...
            <div className="hub-form-row">
              ...
              <div
                className={`hub-toggle ${editForm.isRemote ? 'remote-selected' : 'inperson-selected'}`}
              >
                <button
                  className={`hub-toggle-btn ${editForm.isRemote ? 'active' : ''}`}
                  onClick={() => setEditForm((prev) => ({ ...prev, isRemote: true }))}
                >
                  Remote
                </button>
                <button
                  className={`hub-toggle-btn ${!editForm.isRemote ? 'active' : ''}`}
                  onClick={() => setEditForm((prev) => ({ ...prev, isRemote: false }))}
                >
                  In Person
                </button>
              </div>
            ...
            {editForm.isRemote ? (
              <div className="hub-form-row">
                ...
                <input
                  ...
                  onChange={(e) => setEditForm((prev) => ({ ...prev, zoomLink: e.target.value }))}
                  placeholder="https://..."
                />
              </div>
            ) : (
              <>
                <div className="hub-form-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({ ...prev, locationName: e.target.value }))
                    }
                ...
                <div className="hub-form-row">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({ ...prev, directions: e.target.value.slice(0, 250) }))
                    }
                    rows={3}
                  />
                </div>
            ...
            <div className="hub-form-row">
              ...
              <input
                value={editForm.meetupUrl}
                onChange={(e) => setEditForm((prev) => ({ ...prev, meetupUrl: e.target.value }))}
            ...
            <div className="hub-modal-actions">
              ...
              <button className="hub-btn primary" onClick={handleUpdate} disabled={updating}>
